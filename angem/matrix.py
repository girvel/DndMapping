...
class Matrix:
    ...
    @property
    def size(self):
        return (len(self.strings), len(self.strings[0])) if self.strings else (0, 0)
    ...
    def __imul__(self, other):
        if isinstance(other, Matrix):
            ...
            for v, _ in self.enumerate():
                for i in range(other.size[0]):
                    self[v] += old_strings[v.y][i] * other[i, v.x]
        else:
            for v, _ in self.enumerate():
                self[v] *= other
        return self
...
#         return self != zero
    class Iterator:
        ...
        def __next__(self):
            ...
            if self.enumerated:
                return Vector2(self.x, self.y), self.matrix[self.y, self.x]
